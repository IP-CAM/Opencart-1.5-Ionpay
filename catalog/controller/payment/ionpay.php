...
class ControllerPaymentIONPay extends Controller {
	protected function index() {
		...
		$this->data['action'] = $this->url->link('payment/ionpay/send');

		$this->load->model('checkout/order');
		
		$order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
		
		$this->data['ap_merchant'] = $this->config->get('ionpay_merchant');

		$this->data['url_web'] = $this->url;
		$this->data['ap_security'] = $this->config->get('ionpay_security');
		$this->data['ap_payment'] = $this->config->get('ionpay_payment');
		//$this->data['ap_ionpay_rate'] = $this->config->get('ionpay_rate');
		$this->data['ap_inv_payment'] = $this->config->get('ionpay_inv_payment');
	...
	public function send() {
		...
		$transaction_details['amount'] = $order_info['total'];
		...
		if ($this->config->get('config_currency') != 'IDR') {
			if ($this->currency->has('IDR')) {
				...
				$this->data['errors'][] = 'Currency IDR tidak terinstall atau Ionpay currency conversion rate tidak valid. Silahkan check option kurs dollar.';
			}
		}
